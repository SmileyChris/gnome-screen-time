...
// exactly the bug this pairing exists to prevent: a deleted client's
// session never reaches the exported file, so it must never pick up
// exportedAt either, even though it was in the same period.
test('ClockStore: selectExportable + markExported, run in sequence, stamps only the session that produced a row', () => {
    let clock = freshClock();
    let t = at(2026, 9, 11, 9, 0);
    let known = clock.start('ACME', t);
    clock.stop(t + 3600000);
    let deleted = clock.start('Gone', t + 3600000);
    clock.stop(t + 7200000);

    // 'Gone' was clocked, then deleted from the client list.
    let clients = [{ name: 'ACME', active: true }];
    ...
    assertEqual(clock.sessionById(known.id).exportedAt, stampMs,
        'the known client\'s session produced a row and was recorded as exported');
    assertEqual(clock.sessionById(deleted.id).exportedAt, null,
        'the deleted client\'s session never produced a row, so it must not be stamped either');
    clock.destroy();
});
