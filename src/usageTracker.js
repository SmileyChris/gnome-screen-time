...
export class UsageTracker {
    ...
    // a minute. Otherwise go away until the next input. Observed on GNOME 50
    // Wayland: Mutter already withholds idle watches while idle is
    // inhibited, so this check rarely runs there; it stays as the backstop
    // for sessions where it does not.
    _onIdle() {
        if (this._idle)
            return;
