...
- **Activity breakdown:** inside a terminal running [zellij](https://zellij.dev), time splits by the focused pane's command and working directory; in Brave, Chrome, Firefox and Zen with the companion extension, by site and a site unit such as a repository, subreddit, wiki article, package or tracker project. Expand any row to see it.
...
- **Client clock:** clock in and out per client from the panel or a keyboard shortcut, with the recorded activity kept beside each session as evidence so hours can be adjusted against what actually happened.
...
| Clients | none | Who the clock tracks time for. Inactive ones stay out of the popup but still export. Deleting a client (rather than turning it inactive) asks for confirmation, since it stops any of its recorded sessions from being exported again unless the same name is added back. |
...
- In **Brave, Chrome, Firefox and Zen**, with the companion WebExtension installed (see `companion/README.md`), time is further broken down by site and, where the path names one, by the unit that matters there: repository on code hosts, subreddit on reddit, article on Wikipedia, package on npm, PyPI, crates.io and RubyGems, project key on Jira, team prefix on Linear. Only the site and a short path key (at most three segments) leave the browser, over a local native-messaging host into the extension. Private windows are counted as the browser alone.
...
- **Panel:** *Client time* shows the running session's own elapsed time (not the day's total, so switching clients shows the new one's time from zero), the paused client and its time today, faded, while paused, and nothing once stopped. The panel icon turns into a stopwatch while the clock runs, in every mode, and dims while you're away.
- **Popup:** the clock card sits beside the screen time card at the top, purple while a clock is running and gray otherwise. Running, it shows the client and that client's time today, with a pause button, plus a small session count after the time once that client has two or more sessions today (the panel shows only the running session's own time, so the two differ then); paused, the same with stop and play buttons. Tapping the card itself also pauses or resumes. Resuming from the card, by tapping it or its play button, closes the popup. Stop ends the session too, but forgets the client: the card then shows today's clocked total and can't resume (tapping it opens the Timesheet at today instead, when there's any time), and the panel hides the time until you pick a client from the rows below. Pausing and stopping both close the session the same way; the difference is only what resumes. The keyboard shortcut and DBus `StopSession` pause. In the client rows, the icon after a name shows what tapping it does, like the card's buttons: pause on the running client, play on the paused one. The last row, **Add client…**, opens Preferences with the *Add a client* field focused, and shows even before any client exists. The clock rows, and the card's client and controls, only show for today. Paging back to an earlier day replaces them with that day's clocked total and no controls, and tapping that card opens the Timesheet at that day when it has any time.
- **Under a minute:** a session that ends less than a minute after it started - a mis-tap, or a client you switched away from straight away - is dropped rather than kept, unless you gave it an hours override or a note. That holds however it ends, crash recovery and logout included. Sessions like this saved before this rule existed are cleared out the next time the clock loads; an exported session is always kept.
- **Restart within a minute:** pausing or stopping a client and starting it again within a minute continues the same session, gap included, rather than opening a new one. The same goes for switching away for under a minute and back. It starts a new session instead if the old one has been exported, has an hours override, ended at shutdown, is on another day, or another session came after it. **Trim sleep** and **Trim away time** always start a new session.
...
- **Timesheet:** opened from the popup's footer, it lists sessions from the first day of last month to now, grouped by day, with each day's heading giving every client's hours and, when more than one client worked, the total - the same range "Last month" can export, so the start of a period reachable by export is never out of view here. Expanding one shows the recorded activity behind it (from the interval log) and its Started/Ended times, which accept `HH:MM` and resolve to whichever of the day before, the same day, or the day after is closest to the field's current value, so a start can move back across midnight. Snap buttons move the start to the first recorded activity or to the end of the session before it. **Hours** takes an override; **Use actual** clears it back to the clock's own time; **Save** sends only what you actually changed, and an edit you haven't saved yet survives the window refreshing under you.
- **Export:** from the Timesheet, **Last month** or **This month** as JSON or CSV, chosen by the file's extension: one row per client per day, keyed by a stable `screen-time:{client}:{date}` id that the invoicing side must upsert on, so exporting the same period again is how a correction reaches it. A day with zero hours is still exported, as `0`. If any closed session in the period belongs to a client no longer on the list at all (deleted, not merely made inactive), the toast says how many were skipped. Re-export can't fix everything, though: deleting every session for a client-day, or renaming a client, leaves a stale row behind on the invoicing side with nothing in a later export to correct it. Moving an exported session's start to a different day is refused outright rather than left to cause this: it would create a new day's row on the next export while the old day's row on the invoicing side kept its hours, exporting it twice. Change it in the invoicing app first, or adjust the hours/note instead, both of which stay editable after export.
...
make clean
make companion-build      # build the browser companion for Brave, Chrome, Firefox and Zen
make companion-install    # register its native host with those browsers
```
