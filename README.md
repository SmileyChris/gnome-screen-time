...
- **Popup:** the clock card sits beside the screen time card at the top, purple while a clock is running and gray otherwise; tap it to stop the running clock or start the last client again. The clock rows, and the card's client and tap-to-toggle, only show for today. Paging back to an earlier day replaces them with that day's already-billed total and no Start/Stop control.
