...
|---|---|---|
| Show in panel | Screen time | *Screen time* shows today's total; *Client time* shows the running session's own elapsed time, or today's clocked total once it's stopped; *Nothing* shows only the icon. The icon turns into a stopwatch while the clock runs, in every mode. |
