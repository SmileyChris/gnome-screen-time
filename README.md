...
- **Popup:** the clock card sits beside the screen time card at the top, purple while a clock is running and gray otherwise. Running, it shows the client and that client's time today, with a pause button; paused, the same with stop and play buttons. Tapping the card itself also pauses or resumes. Resuming from the card, by tapping it or its play button, closes the popup. Stop ends the session too, but forgets the client: the card then shows today's clocked total and can't resume (tapping it opens the Timesheet instead), and the panel hides the time until you pick a client from the rows below. Pausing and stopping both close the session the same way; the difference is only what resumes. The keyboard shortcut and DBus `StopSession` pause. In the client rows, the icon after a name shows what tapping it does, like the card's buttons: pause on the running client, play on the paused one. The last row, **Add client…**, opens Preferences with the *Add a client* field focused, and shows even before any client exists. The clock rows, and the card's client and controls, only show for today. Paging back to an earlier day replaces them with that day's clocked total and no controls, and tapping that card opens the Timesheet too.
