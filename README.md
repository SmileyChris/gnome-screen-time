...
|---|---|---|
| Show in panel | Screen time | *Screen time* shows today's total; *Client time* shows the running session's own elapsed time, or today's clocked total once it's stopped; *Nothing* shows only the icon. A dot marks a running clock in every mode. |
...
- **Panel:** *Client time* shows the running session's own elapsed time (not the day's total, so switching clients shows the new one's time from zero), today's clocked total, faded, while paused, and nothing once stopped. A dot marks a running clock in every mode, hollow while you're away and filled otherwise.
