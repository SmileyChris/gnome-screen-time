...
- In **Brave, Chrome, Firefox and Zen**, with the companion WebExtension installed (see `companion/README.md`), time is further broken down by site and, on sites with a rule, by the unit that matters there: repository on code hosts, subreddit on reddit, article on Wikipedia, package on npm, PyPI, crates.io and RubyGems, project key on Jira, team prefix on Linear. Only the site and a short path key (one or two segments) leave the browser, over a local native-messaging host into the extension. Private windows are counted as the browser alone.
